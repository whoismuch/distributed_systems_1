...
int send(void * self, local_id dst, const Message * msg)
{
    ...
    write(((int*) self)[dst], msg, sizeof(msg->s_header) + sizeof(msg->s_payload));
    return 0;
}
...
void close_not_my_pipes(int process_number, int pipes_to_read[process_number+2][process_number+2], int pipes_to_write[process_number + 2][process_number + 2], local_id pid) {
    for (int i=0; i<=process_number; i++) {
        if (i!=pid) {
            for (int j=0; j <= process_number; j++) {
                if (i!=j) {
                    close(pipes_to_read[i][j]);
                    close(pipes_to_write[i][j]);
...
int receive_msgs(int process_number, int pipes_to_read[process_number + 2][process_number + 2], int pid, FILE *p_log) {
    for (int j = 1; j <= process_number; j++) {
        if (pid != j) {
            Message message;
            receive(pipes_to_read[pid], j, &message);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i <= process_number; i++) // loop will run n times (n=5)
    {
        ...
        if (child_pid == 0) {
            close_not_my_pipes(process_number, pipes_to_read, pipes_to_write, i);
            printf(log_started_fmt, i, getpid(), getppid());
            fprintf(e_log, log_started_fmt, i, getpid(), getppid());



            char *message_content = (char*)malloc(snprintf(NULL, 0, log_started_fmt, i, getpid(), getppid()));
            sprintf(message_content, log_started_fmt, i, getpid(), getppid());

            MessageHeader message_header = {
                    .s_magic = MESSAGE_MAGIC,
                    .s_payload_len = strlen(message_content),
                    .s_type = STARTED,
            ...
            strcpy(message.s_payload, message_content);

            send_multicast(pipes_to_write[i], &message);
            ...
            char *message_content2 = (char*)malloc(snprintf(NULL, 0, log_done_fmt, i));
            ...
            MessageHeader message_header2 = {
                    .s_magic = MESSAGE_MAGIC,
                    .s_payload_len = strlen(message_content2),
                    .s_type = DONE,
                    .s_local_time = time(NULL)
            };

            Message message2 = {
                    .s_header = message_header2,
                    .s_payload = {0}
            };

            strcpy(message.s_payload, message_content2);

            send_multicast(pipes_to_write[i], &message2);
    ...
    receive_msgs(process_number, pipes_to_read, 0, p_log);
    receive_msgs(process_number, pipes_to_read, 0, p_log);
